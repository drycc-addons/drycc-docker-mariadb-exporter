...
docker-build: check-docker
	docker build ${DOCKER_BUILD_FLAGS} -t ${IMAGE} ${MYSQLD_EXPORTER_VERSION}/debian
	docker tag ${IMAGE} ${MUTABLE_IMAGE}

docker-buildx: check-docker
	docker buildx build --platform ${PLATFORM} -t ${IMAGE} ${MYSQLD_EXPORTER_VERSION}/debian --push
